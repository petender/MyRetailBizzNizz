  ...
  public class FindLicensePlateText
  {
    ...
    private async Task<string> MakeOCRRequest(byte[] imageBytes)
    {
      ...
      try
      {
        ...
        var ocrResult = Newtonsoft.Json.JsonConvert.DeserializeObject<OCRResult>(result) ?? throw new Exception("Could not deserialize the response to an OCRResult object.");
